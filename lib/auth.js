...
function writeAdminFile(username, password) {
...
function ensureAdminFile() {
  if (RESET_ADMIN) {
    if (fs.existsSync(ADMIN_FILE)) fs.unlinkSync(ADMIN_FILE);
    console.log("[auth] ADMIN_RESET=true -> รีเซ็ตบัญชีแอดมิน");
  }

  let needsCreate = !fs.existsSync(ADMIN_FILE);
  ...
  if (needsCreate) {
    writeAdminFile(DEFAULT_USERNAME, DEFAULT_PASSWORD);
    console.log(
      `[auth] พร้อมใช้งาน -> username: "${DEFAULT_USERNAME}" password: "${DEFAULT_PASSWORD}"`
    );
  }
}
